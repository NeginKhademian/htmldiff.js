...
if (token) {
    ...
    function calculateOperations(beforeTokens, afterTokens){
        ...
        for (var j = 0; j < postProcessed.length; j++){
            ...
            if (lastOp && op.action === 'replace' && lastOp.action === 'replace') {
                // Merge if together they form a single word in before and after
                var mergedStartInBefore = lastOp.startInBefore;
                var mergedEndInBefore = op.endInBefore;
                var mergedStartInAfter = lastOp.startInAfter;
                var mergedEndInAfter = op.endInAfter;
                var beforeStr = beforeTokens.slice(mergedStartInBefore, mergedEndInBefore + 1).map(t=>t.string).join('');
                var afterStr = afterTokens.slice(mergedStartInAfter, mergedEndInAfter + 1).map(t=>t.string).join('');
                if (/^[^\s]+$/.test(beforeStr) && /^[^\s]+$/.test(afterStr)) {
                    lastOp.endInBefore = op.endInBefore;
                    lastOp.endInAfter = op.endInAfter;
                    continue;
                } else {
                    finalOps.push(lastOp);
                }
            } else if (lastOp) {
                finalOps.push(lastOp);
            }
        ...
        if (lastOp) finalOps.push(lastOp);
        return finalOps;
    }
...
function groupTokensByWord(tokens) {
    // Group tokens into words (join consecutive non-whitespace tokens)
        ...
        for (var j = 0; j < postProcessed.length; j++){
            var op = postProcessed[j];
            // Aggressively merge consecutive replaces if together they form a single word in before and after
            if (op.action === 'replace' && lastOp.action === 'replace') {
                var mergedStartInBefore = lastOp.startInBefore;
                var mergedEndInBefore = op.endInBefore;
                var mergedStartInAfter = lastOp.startInAfter;
                var mergedEndInAfter = op.endInAfter;
            ...
            } else if (lastOp.action !== 'none') {
                finalOps.push(lastOp);
            }
            lastOp = op;
        }
        if (lastOp && lastOp.action !== 'none') finalOps.push(lastOp);
        return finalOps;
    },
    'insert': function(op, beforeTokens, afterTokens, opIndex, dataPrefix, className){
        var tokens = afterTokens.slice(op.startInAfter, op.endInAfter + 1);
    ...
    'replace': function(op, beforeTokens, afterTokens, opIndex, dataPrefix, className){
        ...
        // If both are text, use word-level diff, but for any contiguous non-whitespace sequence, wrap the whole sequence
        if (tokensAreAllText(before) && tokensAreAllText(after)) {
            var beforeWord = before.map(t=>t.string).join('');
            var afterWord = after.map(t=>t.string).join('');
            // If both are contiguous non-whitespace sequences and differ, wrap the whole sequence
            if (/^[^\s]+$/.test(beforeWord) && /^[^\s]+$/.test(afterWord) && beforeWord !== afterWord) {
                return wrap('del', [beforeWord], opIndex, dataPrefix, className) + wrap('ins', [afterWord], opIndex, dataPrefix, className);
            }
            // Otherwise, fallback to word-level diff
