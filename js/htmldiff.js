...
(function () {
     ...
     */
    // --- Replace isWrappable with this ---
    function isWrappable(token) {
        var is_img = /^<img[\s>]/i.test(token);
        var tag = isTag(token); // falsy for text nodes, string for tags (e.g. 'p' or '/p')

        // Atomic or void tags, or <img>, are safe to wrap
        if (is_img || isStartOfAtomicTag(token) || isVoidTag(token)) {
            return true;
        }

        // Tags (opening or closing) can be wrapped; text nodes too
        if (tag) {
            return true;
        }

        // For text: only wrap if it has non-whitespace content
        return /\S/.test(token.replace(/&nbsp;|&#160;/g, ' '));
    }
    ...
    function getKeyForToken(token) {
        ...
        var t = token.trim();
        var tagNameMatch = /^<\/?([^\s>\/]+)/.exec(t);
        if (tagNameMatch) {
            var tagName = tagNameMatch[1].toLowerCase();
            return /^<\//.test(t) ? '</' + tagName + '>' : '<' + tagName + '>';
        }
